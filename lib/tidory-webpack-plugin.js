...
module.exports = class {
  ...
  apply (compiler) {
    compiler.hooks.compilation.tap('TidoryWebpackPlugin', compilation => {
      HtmlWebpackPlugin.getHooks(compilation).beforeEmit.tapAsync('TidoryWebpackPlugin', (data, callback) => {
        this.$ = cheerio.load(data.html)

        data.html = this.setStyle(this.css())
          .setScript(this.js())
          .html()
   ...
   */
  css () {
    let extractedStyle = this.extract('style')

    extractedStyle = extractedStyle.replace(
      /content:\s?'([^\\]*?)'/gim,
      (_, content) => `content:'${cssesc(content)}'`
    )

    return css(extractedStyle, this.beautifierOptions().css)
  }
   ...
   */
  js () {
    let extractedScript = this.extract('script:not([src])')

    extractedScript = babel.transformSync(extractedScript, {
      presets: [[require.resolve('@babel/preset-env'), { modules: false }]]
    }).code

    return js(extractedScript, this.beautifierOptions().js)
  }
